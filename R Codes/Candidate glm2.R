...
allpossregs(`Candidate Vote`~log(`Party Vote Electorate`)+Incumbent+`Same Party`*(log(`Candidate Previous Vote`)+log(`Previous Candidate Vote electorate`)), family = log(gaussian), data=Mixed.df)
...
FirstTimers.fit <- glm(`Candidate Vote`~log(`Party Vote Electorate`), family = gaussian(log), data = NewCand.df)
plot(FirstTimers.fit, which = 1:6)

# Issue is underesimate of Colin Craig by ~16/17% perhaps add a party leader boost?
...
## FIX ## 06/06 using log gaussian family as suggested by Thomas
...
### EDIT 6/06: Doing the log gaussian shennagians

allpossregs(`Candidate Vote`~log(`Party Vote Electorate`)*Party+log(`Vote to Use`)*(`Same Electorate`+`Same Candidate`+`Same Party`)+Incumbent, data = RestB14.df)

# Now dropping stuff not imp:

allpossregs(`Candidate Vote`~log(`Party Vote Electorate`)*Party+log(`Vote to Use`)*(`Same Party`)+Incumbent, data = RestB14.df)
...
Candidate.fit <- glm(`Candidate Vote`~log(`Party Vote Electorate`)*Party+log(`Vote to Use`):`Same Party` + `Incumbent`, family = gaussian(log), data = RestB14.df)
...
Candidate.fit1 <- glm(`Candidate Vote`~log(`Party Vote Electorate`)*Party+log(`Vote to Use`):`Same Party` + `Incumbent`, family = gaussian(log), data = RestB14.df)
...
Candidate.fit2 <- glm(`Candidate Vote`~log(`Party Vote Electorate`)*Party+log(`Vote to Use`)+log(`Vote to Use`):`Same Party` + `Incumbent`, family = gaussian(log), data = RestB14.df)
plot(Candidate.fit2, which = 1:6)

Candidate.fit3 <- glm(`Candidate Vote`~log(`Party Vote Electorate`)*Party+log(`Vote to Use`) + `Incumbent`, family = gaussian(log), data = RestB14.df)
...
# In the Interaction models Jim Anderton 1999 and Winston 2002 spanners but probably won't drop Winnie. Dropped Jim not Winnie -> also Ricahrd Prebble Wellington Central 99 an issue as Nat ran no cand and so he got all their vote (Drop?) Decided to drop him
...
allpossregs(`Candidate Vote`~log(`Party Vote Electorate`)+log(`Vote to Use`)+Incumbent, family = gaussian(log), data = Ohariu.df)

Ohariu.fit <- glm(`Candidate Vote`~log(`Party Vote Electorate`)+log(`Vote to Use`)+Incumbent, family = gaussian(log), data = Ohariu.df)

# Below is best from all poss regs but won't deal well with labour cand therefore will use above

Ohariu.fit1 <- glm(`Candidate Vote`~log(`Party Vote Electorate`)+Incumbent, family = gaussian(log), data = Ohariu.df)
...
Epsom.fit1 <- glm(`Candidate Vote`~log(`Party Vote Electorate`)+ACT+National, family = gaussian(log), data = Epsom.df)
Epsom.fit2 <- glm(`Candidate Vote`~log(`Vote to Use`)+log(`Party Vote Electorate`)*National+ACT, family = gaussian(log), data = Epsom.df)

# Not great but hopefully works for 2014 either 1 or 2?
...
write.csv(Pred14.df, "GaussianLog.csv")
